...
def load(f: _BinaryIO, extensions: Extension | None = None, pure_data: bool = False, protocol: int = _version) -> _Any:
    '''
...
def loads(source: bytes | bytearray, extensions: Extension | None = None, pure_data: bool = False, protocol: int = _version) -> _Any:
    '''
    ...
    Raises:
        ...
        ExtensionNotFoundError: If an extension is not found
    '''
    return load(_BytesIO(source), extensions, pure_data, protocol)
            
def dump(obj: _Any, f: _BinaryIO, extensions: Extension | None = None, protocol: int = _version, pure_data: bool = False):
    '''
    ...
    '''
    try:
        if protocol >= 5: return _import(f'.pcls.core_p{protocol}', __package__)._dump(obj, f, extensions, pure_data)
        else: return _import(f'.pcls.core_p{protocol}', __package__)._dump(obj, f, extensions)
    except ModuleNotFoundError: raise ValueError(f"unknown protocol: {protocol}") from None


def dumps(obj: _Any, extensions: Extension | None = None, protocol: int = _version, pure_data: bool = False) -> bytes:
    '''
    ...
    '''
    buf = _BytesIO()
    dump(obj, buf, extensions, protocol, pure_data)
    return buf.getvalue()
