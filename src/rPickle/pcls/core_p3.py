...
from typing import BinaryIO as _BinaryIO, Any as _Any, Callable as _Callable, Iterable as _Iterable, Mapping as _Mapping
...
def _fix(obj):
    ...
    while stack:
        ...
        if isinstance(obj, _Mapping):
            if _hashable(obj): stack += (i for i in obj.values() if id(i) not in seen)
            else:
                for k, v in *obj.items(),:
                    if _hashable(v) and isinstance(v, _R): obj[k] = v.x
                    elif id(v) not in seen: stack.append(v)
        elif isinstance(obj, _Iterable):
            if _hashable(obj): stack += (i for i in obj if id(i) not in seen)
            else:
                for i in range(len(obj)):
                    if not _hashable(obj[i]):
                        if id(obj[i]) not in seen: stack.append(obj[i])
                        continue
...
def _load(f: _BinaryIO, extensions: Extension | None = None) -> _Any:
    ...
    def make_range(items: list) -> range:
        if len(items) != 3: raise EOFError(f"range() expected at 3 args, got {len(items)}")
        return range(*items)
    def make_slice(items: list) -> slice:
        if len(items) != 3: raise EOFError(f"slice() expected 3 args, got {len(items)}")
        return slice(*items)
    def make_complex(items: list) -> complex:
        if len(items) != 2: raise EOFError(f"complex() expected 2 args, got {len(items)}")
        ...
        match opcode:
            ...
            case 0x11:
                ...
                if has_real and has_imag:
                    stack.append([UNSET, idx, 2, [], make_complex])
                elif has_real:
                    stack.append([UNSET, idx, 1, [], lambda items: complex(items[0], 0)])
                elif has_imag:
                    stack.append([UNSET, idx, 1, [], lambda items: complex(0, items[0])])
                else:
                    v = 0j
                    objs_id[idx] = v
                    push_value(v)
            ...
            case 0x12 | 0x13 as code:
                idx = len(objs_id)
                objs_id.append(None)
                stack.append([UNSET, idx, 3, [], make_range if code == 0x12 else make_slice])
            ...
            # system
            case 0xF0:
                value = objs_id[read(read(1, True), True)]
                if isinstance(value, _R):
                    value = value.x
                    if isinstance(value, _R): need_to_fix = True
                    push_value(value)
    ...
    while stack:
        ...
        match obj:
            ...
            case sentinel():
                ...
                f.write(len(content).to_bytes())
                f.write(content)
                f.write(id(obj).to_bytes(8, 'little'))
