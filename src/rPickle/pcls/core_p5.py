...
def _load(f: _BinaryIO, extensions: Extension | None = None) -> _Any:
    ...
    def read(length: int, need_int: bool = False, signed: bool = False) -> bytes | int:
        ...
        data = f.read(length)
        if len(data) != length: raise EOFError(f"expected at least {length} byte{'s' * (length > 1)}, got {len(data)} byte{'s' * (length > 1)}, at {f.tell()}")
    ...
    def readinto(length: int) -> bytearray:
        ...
        n = f.readinto(data)
        if n != length: raise EOFError(f"expected at least {length} byte{'s' * (length > 1)}, got {len(data)} byte{'s' * (length > 1)}, at {f.tell()}")
        return data
    ...
    while result is UNSET:
        ...
        match opcode:
            ...
            case _: raise ValueError(f"Unknown opcode: {opcode:#x}, at {f.tell()}")
