...
class TestBasicTypes:
    ...
    def test_int(self):
        for n in [-1, 0, 1, 127, 128, 32767, 32768, (1 << 31) - 1, 1 << 31, 1 << 64, 1 << (1 << 16)]:
    ...
    def test_complex(self):
        for c in [1+2j, 3.5-4j, 1j, 0j, 3.14+0j]:
    ...
    def test_bytearray(self):
        ...
        ba = bytearray(b'\x00\x01\x02\xFF') * 100
        result = rPickle.loads(rPickle.dumps(ba))
        assert result == ba
        assert isinstance(result, bytearray)
    ...
    def test_range(self):
        rs = [range(0), range(5), range(1, 2, 3), range(5, -1, -1), range(1 << 63, 1 << 64, 4)]
        for r in rs: assert rPickle.loads(rPickle.dumps(r)) == r

    def test_slice(self):
        ss = [slice(0), slice(5), slice(1, 2, 3), slice(5, -1, -1), slice(1 << 63, 1 << 64, 4)]
        for s in ss: assert rPickle.loads(rPickle.dumps(s)) == s

    @pytest.mark.skipif(sys.version_info < (3, 15), reason='requires Python 3.15+')
    def test_sentinel(self):
        SEN = sentinel('SEN')
        assert rPickle.loads(rPickle.dumps(SEN)) is SEN
